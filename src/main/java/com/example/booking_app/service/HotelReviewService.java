...
public class HotelReviewService {
    ...
    public List<HotelReviewResponse> getAllReviews(Long hotelId) {
        Hotel hotel =
                hotelRepository.findById(hotelId).orElseThrow(() -> new AppException(ErrorCode.HOTEL_NOT_EXISTED));
        return hotelReviewRepository.findByHotel(hotel).stream()
                .map(hotelReview -> {
    ...
    public HotelReviewResponse createReview(HotelReviewRequest request) {
        ...
        HotelReviewResponse hotelResponse =
                hotelReviewMapper.toHotelReviewResponse(hotelReviewRepository.save(hotelReview));
